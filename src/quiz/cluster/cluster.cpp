...
void clusterHelp(int index, const std::vector<std::vector<float>> points ,std::vector<int>& cluster,std::vector<bool>& processed, KdTree* tree, float distanceTol)
{
	//if not visited
	if(processed[index] == false){
		...
		//int index = distance(processed.begin(), processed.find(point));
		cluster.push_back(index);
		// find nearby points 
		// tree search
		std::vector<int> ids = tree -> search(points[index], distanceTol);
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>> points, KdTree* tree, float distanceTol)
{
	...
	//std::map<std::vector<float>,bool> processed;
	std::vector<bool> processed(points.size(), false); 

	//for(std::vector<float> point : points){
	//	processed.insert(std::pair<std::vector<float>,bool>(point,false));
	//}
	int i = 0;
	for(std::vector<float> point : points){
		// new cluster
		if( processed[i++] == false){ //if this point has not visited
			//create cluster
			//cout<<"i:"<<i;
